...
data = []
username = ["htv-nghiadv1", "htv-sidd", "htv-hienntt", "htv-loidh", "htv-manhld", "htv-vietlq", "htv-anhndt", "htv-quangdd", "htv-sonnv"]
usernameDisplay = ["NghiaDV", "SiDD", "HienNTT", "LoiDH", "ManhLD", "VietLQ", "AnhNDT", "QuangDD", "SonNV"]
user_display_map = dict(zip(username, usernameDisplay))
print(f"Hello: {', '.join(username)} - ({', '.join(usernameDisplay)}). Scanning...")
...
for repo in repos:
    ...
    for pr in tqdm(prs, desc=f"{repo} - Page {page}"):
        ...
        if html_url not in exclude_prs and pr_login in user_display_map:
            ...
            data.append({
                "Repo": repo,
                "Title": pr.get("title"),
                "Author": user_display_map.get(pr_login, pr_login),
                "URL": pr.get("html_url"),
...
if data:
    ...
    df = df.sort_values(by=["Author"], na_position="last").reset_index(drop=True)
